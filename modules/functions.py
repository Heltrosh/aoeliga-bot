...
#EXCUSE CMD FUNCTIONS
def excuseList():
    # Creates a string to be sent by the "!excuse list" command
    excuseListOutput = "**ZOZNAM OSPRAVEDLNENÝCH HRÁČOV**\n"
    leagueHeaderPrinted = [0, 0, 0, 0, 0, 0]
    players = getPlayers()
    sortedPlayers = sorted(players, key=lambda x: x[3]) #sorts players by their league so they can be printed in descending order
    for player in sortedPlayers:
        if player[2]: #if player is excused from any round
            if leagueHeaderPrinted[player[3]-1] == 0:
                if not 1 in leagueHeaderPrinted:
                    excuseListOutput += (f"**{str(player[3])}. LIGA:**\n")
                else:
                    excuseListOutput += (f"\n**{str(player[3])}. LIGA:**\n")
